...
export default function BusinessEditPage() {
  ...
  useEffect(() => {
    if (business && user) {
      if (business.telephone) setTelephone(business.telephone);
      if (business.cellphone) setCellphone(business.cellphone);
      if (business.store_name) setStore_name(business.store_name);
      if (business.store_address) setStore_address(business.store_address);
      if (business.remark) setRemark(business.remark);
      if (business.price) setPrice(business.price);
    }
    return () => dispatch(cleanBusiness);
  }, [business]);
  ...
  return (
    ...
    >
      ...
      <Div>
        <Text textSize="display1">編輯生意攤資料</Text>
        <Div>
          <Div justify="center">
